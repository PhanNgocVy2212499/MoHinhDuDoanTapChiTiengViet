...
Dự án này xây dựng và so sánh ba mô hình baseline cho bài toán phân loại chủ đề tin tức tiếng Việt.
...
- So sánh giữa mô hình xác suất đơn giản (Naive Bayes), mô hình phân lớp biên mạnh (Linear SVM) và mô hình cây quyết định (Decision Tree).
...
- train_dt_progress.py: huấn luyện riêng mô hình Decision Tree, in tiến độ theo từng bước, in metrics và lưu confusion matrix.
- RunTrain.py: chạy tuần tự NB, SVM, Decision Tree; gom kết quả và xuất bảng, biểu đồ so sánh.
...
python train_dt_progress.py --csv dataset_tapchi.csv --save-cm confusion_matrix_dt.png
```

### 4.2 Chạy tổng hợp 3 mô hình (khuyến nghị)
...
Ba mô hình dùng cùng một pipeline tiền xử lý để đảm bảo so sánh công bằng:
...
Decision Tree học các quy tắc tách dữ liệu theo từng đặc trưng để phân lớp.
Trong bài toán này, mô hình dùng tham số giới hạn độ sâu để tránh cây quá lớn.
...
- Mỗi nút cây là một câu hỏi tách dữ liệu theo một đặc trưng TF-IDF.
- Dự đoán cuối cùng được đưa ra ở lá theo lớp chiếm ưu thế.

Ưu điểm:

- Dễ hiểu và dễ giải thích.
- Có thể biểu diễn trực quan theo dạng cây quyết định.

Hạn chế:

- Trên dữ liệu văn bản nhiều chiều, hiệu năng thường thấp hơn SVM.
- Thời gian huấn luyện có thể cao khi số đặc trưng lớn.

## 7. Kết quả thực nghiệm baseline

Số liệu dưới đây lấy từ lần chạy gần nhất của RunTrain.py:

| Mô hình        | Accuracy | Precision (weighted) | Recall (weighted) | F1-score (weighted) | Thời gian (giây) |
| -------------- | -------- | -------------------- | ----------------- | ------------------- | ---------------- |
| Naive Bayes    | 0.6963   | 0.7057               | 0.6963            | 0.6525              | 94.93            |
| SVM (Linear)   | 0.7995   | 0.7925               | 0.7995            | 0.7945              | 121.35           |
| Decision Tree  | 0.6285   | 0.6287               | 0.6285            | 0.6284              | 906.44           |

Phân tích chi tiết:

- Accuracy: SVM cao nhất, gần mốc 80%, cho thấy mô hình phân lớp tổng thể tốt nhất trong ba mô hình.
- F1-score weighted: SVM vượt trội so với NB và Decision Tree.
- Decision Tree có độ chính xác thấp hơn và thời gian huấn luyện cao hơn rõ rệt trên không gian TF-IDF lớn.
- Thời gian huấn luyện: NB nhanh nhất, phù hợp khi cần mô hình nhẹ và thời gian chạy ngắn.

Ý nghĩa thực tế:

- Nếu ưu tiên hiệu năng dự đoán: SVM là lựa chọn baseline mạnh nhất.
- Nếu ưu tiên tốc độ huấn luyện hoặc tài nguyên thấp: NB vẫn hữu ích làm mốc ban đầu.
- Decision Tree phù hợp để so sánh thêm tính diễn giải, nhưng chưa phải lựa chọn tốt nhất về hiệu năng trong thí nghiệm hiện tại.
...
- Decision Tree: dễ diễn giải nhưng hiệu năng thấp hơn và tốn thời gian huấn luyện hơn trên bộ đặc trưng TF-IDF.
